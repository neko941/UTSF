...
class FullAttention__Pytorch(nn.Module):
    def __init__(self, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False):
        super(FullAttention__Pytorch, self).__init__()
        self.scale = scale
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask__Pytorch(B, L, device=queries.device)
...
class FullAttention__Tensorflow(tf.keras.layers.Layer):
    def __init__(self, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False):
        super(FullAttention__Tensorflow, self).__init__()
        self.scale = scale
        self.mask_flag = mask_flag
        self.output_attention = output_attention
    ...
    def call(self, queries, keys, values, attn_mask):
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / tf.sqrt(E)

        scores = tf.einsum("blhe,bshe->bhls", queries, keys)

        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask__Tensorflow(B, L)

            scores = tf.where(attn_mask, scores, tf.constant(-np.inf, dtype=scores.dtype))
...
class ProbAttention__Pytorch(nn.Module):
    def __init__(self, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False):
        super(ProbAttention__Pytorch, self).__init__()
        self.factor = factor
        self.scale = scale
        self.mask_flag = mask_flag
        self.output_attention = output_attention
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        ...
        if self.mask_flag:
            attn_mask = ProbMask__Pytorch(B, H, L_Q, index, scores, device=V.device)
            scores.masked_fill_(attn_mask.mask, -np.inf)
...
class AttentionLayer__Pytorch(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None):
        super(AttentionLayer__Pytorch, self).__init__()
...
class AttentionLayer__Tensorflow(tf.keras.layers.Layer):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None):
        super(AttentionLayer__Tensorflow, self).__init__()
